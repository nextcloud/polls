...
class OptionMapper extends QBMapperWithUser {
	 ...
	 * @return Option
	 * @param int $pollId
	 * @param string $pollOptionHashBin option text as binary hashed value (pollId and PollOptionText)
	 * @param bool $getDeleted also search for deleted options
	 */
	...
	protected function joinVotesCount(
		...
		$qb->leftJoin(
			...
			$qb->expr()->andX(
				...
				$qb->expr()->eq($joinAlias . '.vote_option_hash_bin', $fromAlias . '.poll_option_hash_bin'),
			)
		)
