OC.L10N.register(
    "polls",
    {
    "Polls" : "Anketler",
    "Poll invitation \"%s\"" : "\"%s\" anketine çağrı",
    "{owner} invited you to take part in the poll \"{title}\"" : "{owner} kullanıcısı sizi \"{title}\" anketine katılmaya çağırdı",
    "Go to poll" : "Ankete git",
    "This email is sent to you, because you are invited to vote in this poll by the poll owner." : "Bu e-posta, anket sahibi tarafından bu ankete oy vermeniz için gönderildi.",
    "Polls App - New Activity" : "Anketler Uygulaması - Yeni İşlem",
    "\"{title}\" had recent activity: " : "Son \"{title}\" işlemleri: ",
    "- %s voted." : "- %s oy verdi.",
    "- %s updated the poll configuration. Please check your votes." : "- %s anket yapılandırmasını güncelledi. Lütfen oylarınızı denetleyin.",
    "- %s deleted the poll." : "- %s anketi sildi.",
    "- %s restored the poll." : "- %s anketi geri yükledi.",
    "- The poll expired." : "- Anketin süresi dolmuş.",
    "This email is sent to you, because you subscribed to notifications of this poll. To opt out, visit the poll and remove your subscription." : "Bu e-posta, bu anketin bildirimlerine abone olduğunuz için gönderildi. Bildirimlerin gönderilmemesi için ankete giderek abonelikten ayrılabilirsiniz.",
    "A polls app, similar to doodle/dudle with the possibility to restrict access." : "Doodle/Dudle benzeri, erişim kısıtlama özelliği bulunan bir anket uygulamasıdır.",
    "A polls app, similar to doodle/dudle with the possibility to restrict access (members, certain groups/users, hidden and public)." : "Doodle/Dudle benzeri, erişim kısıtlama özelliği (üyeler, belirli grup ve kullanıcılar, gizli ve herkese açık gibi) bulunan bir anket uygulamasıdır.",
    "Group" : "Grup",
    "Create new poll" : "Anket ekle",
    "Enter Title" : "Başlığı yazın",
    "Poll type" : "Anket türü",
    "Poll schedule" : "Anket zamanlaması",
    "Text based" : "Metin temelli",
    "Cancel" : "İptal",
    "Apply" : "Uygula",
    "Poll \"%n\" added" : "\"%n\" anketi eklendi",
    "Error while creating Poll \"%n\"" : "\"%n\" anketi eklenirken sorun çıktı",
    "Add new Poll" : "Anket ekle",
    "All polls" : "Tüm anketler",
    "My polls" : "Anketlerim",
    "Public polls" : "Herkese açık anketler",
    "Hidden polls" : "Gizli anketler",
    "Deleted polls" : "Silinmiş anketler",
    "Error loading polls" : "Anketler yüklenirken sorun çıktı",
    "Title" : "Başlık",
    "Access" : "Erişim",
    "Owner" : "Sahip",
    "Created" : "Oluşturulma",
    "Expires" : "Sona Erme",
    "Visible to other users" : "Diğer kullanıcılar görebilir",
    "Hidden to other users" : "Diğer kullanıcılar göremez",
    "never" : "asla",
    "Clone poll" : "Anketi kopyala",
    "Delete poll" : "Anketi sil",
    "Delete poll as admin" : "Anketi yönetici olarak sil",
    "Share with" : "Şununla paylaş",
    "Your comment was added" : "Yorumunuz eklendi",
    "Error while saving comment" : "Yorum kaydedilirken sorun çıktı",
    "Details" : "Ayrıntılar",
    "Comments" : "Yorumlar",
    "options" : "seçenekler",
    "Configuration" : "Yapılandırma",
    "Shares" : "Paylaşımlar",
    "No comments yet. Be the first." : "Henüz bir yorum yapılmamış. İlk yorum yapan siz olun.",
    "Saving" : "Kaydediliyor",
    "Saved" : "Kaydedildi",
    "Description" : "Açıklama",
    "Poll configurations" : "Anket yapılandırmaları",
    "Allow \"maybe\" vote" : "\"Belki\" oyu verilebilsin",
    "Anonymous poll" : "Anonim anket",
    "Hide user names for admin" : "Yönetici için kullanıcı adları gizlensin",
    "Restore poll" : "Anketi geir yükle",
    "Expiration date" : "Sona erme tarihi",
    "Writing poll" : "Yazma anketi",
    "Update poll" : "Anketi güncelle",
    "Title must not be empty!" : "Başlık boş olamaz!",
    "%n successfully saved" : "%n kaydedildi",
    "Add a date option" : "Bir tarih seçeneği ekle",
    "Shift all date options" : "Tüm tarih ayarlarını kaydır",
    "Shift" : "Kaydır",
    "Available Options" : "Kullanılabilecek Seçenekler",
    "Click to add a date" : "Bir tarih eklemek için tıklayın",
    "Invitations" : "Çağrılar",
    "Public shares" : "Herkese açık paylaşımlar",
    "Share Link" : "Paylaşım Bağlantısı",
    "Add a public link" : "Herkese açık bir bağlantı ekle",
    "Link copied to clipboard" : "Bağlantı panoya kopyalandı",
    "Error while copying link to clipboard" : "Bağlantı panoya kopyalanırken sorun çıktı",
    "Unknown user" : "Kullanıcı bilinmiyor",
    "Public share" : "Herkese açık paylaş",
    "You added %n." : "%n eklediniz.",
    "Error while adding share" : "Paylaşım eklenirken sorun çıktı",
    "Add a new text option" : "Bir metin seçeneği ekle",
    "Enter option text and press Enter" : "Seçenek metnini yazıp Enter tuşuna basın",
    "Receive notification email on activity" : "İşlem yapıldığında e-posta bildirimi gönderilsin",
    "Deleted" : "Silindi",
    "Enter a valid username, to participate in this poll." : "Bu ankete katılmak için geçerli bir kullanıcı adı yazın.",
    "Enter a valid username with at least 3 Characters" : "En az 3 karakter uzunluğunda geçerli bir kullanıcı adı yazın",
    "You will be redirected to your personal share." : "Kişisel paylaşım alanınıza yönlendirileceksiniz.",
    "If you are not redirected to your poll click this link:" : "Ankete yönlendirilmezseniz şu bağlantıya tıklayın:",
    "Your personal link to this poll: %n" : "Bu anketin size özel bağlantısı: %n",
    "Please use at least 3 characters for your user name!" : "Lütfen kullanıcı adınız için en az 3 karakter yazın!",
    "Place your votes until %n" : "Lütfen %n zamanına kadar oyunuzu verin",
    "Expired since %n" : "Süresi %n zamanında dolmuş",
    "This user name can not be chosen." : "Bu kullanıcı adı seçilemez.",
    "Error saving user name" : "Kullanıcı adı kaydedilirken sorun çıktı",
    "there are no vote Options" : "herhangi bir oy verme seçeneği yok",
    "Click here to add a poll" : "Bir anket eklemek için tıklayın",
    "Do you want to delete \"%n\"?" : "\"%n\" anketini silmek istiyor musunuz?",
    "No, keep poll." : "Hayır, anket korunsun.",
    "Yes, delete poll." : "Evet, anket silinsin.",
    "Poll \"%n\" deleted" : "\"%n\" anketi silindi",
    "Error while deleting Poll \"%n\"" : "\"%n\" anketi silinirken sorun çıktı",
    "Open Sidebar" : "Yan Çubuğu Aç",
    "Public access" : "Herkese açık erişim",
    "Hidden poll" : "Gizli anket",
    "Access denied" : "Erişim engellendi",
    "You are not allowed to view this poll or the poll does not exist." : "Bu anket bulunamadı ya da görüntüleme izniniz yok.",
    "You are not allowed to edit this poll or the poll does not exist." : "Bu anket bulunamadı ya da düzenleme izniniz yok.",
    "You are either not allowed to delete this poll or it doesn't exist." : "Bu anketi silme izniniz yok ya da anket bulunamadı."
},
"nplurals=2; plural=(n > 1);");
