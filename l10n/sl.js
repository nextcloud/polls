OC.L10N.register(
    "polls",
    {
    "Polls" : "Ankete",
    "Poll invitation \"%s\"" : "Vabilo k izpolnjevanju ankete »%s«",
    "{owner} invited you to take part in the poll \"{title}\"" : "{owner} vas vabi k izpolnjevanju ankete » {title} «",
    "Go to poll" : "Skoči na anketo",
    "Polls App - New Activity" : "Program Ankete – nova dejavnost",
    "\"{title}\" had recent activity: " : "Na anketi » {title} « je zaznana dejavnost: ",
    "- %s voted." : "– %s izpolni anketo",
    "- %s updated the poll configuration. Please check your votes." : "– %s posodobi nastavitve ankete. Preverite glasovanje.",
    "- %s deleted the poll." : "– %s izbriše anketo.",
    "- %s restored the poll." : "– %s obnovi anketo.",
    "- The poll expired." : "– Anketa je potekla.",
    "- %s added a vote option." : "– %s doda možnost k anketi.",
    "- %s removed a vote option." : "– %s odstrani možnost ankete.",
    "This email is sent to you, because you subscribed to notifications of this poll. To opt out, visit the poll and remove your subscription." : "Elektronsko sporočilo ste prejeli, ker ste se naročili na obvestila te ankete. Naročila je mogoče kadarkoli odstraniti med možnostmi ankete.",
    "A polls app, similar to doodle/dudle with the possibility to restrict access." : "Nextcloud Polls je program, podoben spletnemu programu Doodle z možnostjo omejevanja dostopa.",
    "A polls app, similar to doodle/dudle with the possibility to restrict access (members, certain groups/users, hidden and public)." : "Nextcloud Polls je program, podoben spletnemu programu Doodle z možnostjo omejevanja dostopa (za člane, skupine ali uporabnike, skrite ankete in javno zbiranje odziva).",
    "Error loading polls" : "Napaka nalaganja anket",
    "Participants" : "Udeleženci",
    "No Participants until now" : "Ni še zabeleženih udeležencev ankete",
    "The names of other participants are hidden, as this is an anonymous poll. " : "Imena udeležencev so skrita, ker je to anonimna anketa.",
    "Results are hidden. " : "Odzivi na anketo so skriti.",
    "_%n person participated in this poll until now. _::_%n persons participated in this poll until now. _" : [" Do sedaj je sodelovala %n oseba."," Do sedaj sta sodelovali %n osebi."," Do sedaj so sodelovale %n osebe."," Do sedaj je sodelovalo %n oseb. "],
    "New comment …" : "Nova opomba ...",
    "Your comment was added" : "Opomba je dodana",
    "You can place your vote until %n. " : "Anketo lahko izpolnite do %n",
    "Error while saving comment" : "Napaka med shranjevanjem opombe",
    "Comments" : "Opombe",
    "Delete comment" : "Izbriši opombo",
    "No comments yet. Be the first." : "Ni še odzivov, bodite prvi!",
    "Comment deleted" : "Opomba je izbrisana",
    "started this poll on %n. " : "objavi vprašalnik: %n. ",
    "Error while deleting the comment" : "Prišlo je do napake med brisanjem opombe",
    "Create new poll" : "Ustvari novo anketo",
    "Enter Title" : "Naslov",
    "Poll type" : "Vrsta ankete",
    "Date poll" : "Usklajevanje dogodka",
    "Text based" : "Besedilna anketa",
    "Cancel" : "Prekliči",
    "Apply" : "Uveljavi",
    "Poll \"%n\" added" : "Anketa » %n « je dodana.",
    "Error while creating Poll \"%n\"" : "Prišlo je do napake med ustvarjanjem ankete » %n «",
    "Add new Poll" : "Dodaj novo anketo",
    "Relevant" : "Pomembno",
    "My polls" : "Moje ankete",
    "Public polls" : "Javne ankete",
    "All polls" : "Vse ankete",
    "Expired polls" : "Potečene ankete",
    "Deleted polls" : "Izbrisane ankete",
    "Clone poll" : "Kloniraj anketo",
    "Delete poll as admin" : "Izbriši anketo kot skrbnik",
    "Delete poll" : "Izbriši anketo",
    "Restore poll as admin" : "Obnovi anketo kot skrbnik",
    "Restore poll" : "Obnovi anketo",
    "Delete poll permanently as admin" : "Trajno izbriši anketo kot skrbnik",
    "Delete poll permanently" : "Trajno izbriši anketo",
    "Title" : "Naslov",
    "Access" : "Dostop",
    "Owner" : "Lastnik",
    "Created" : "Ustvarjeno",
    "Expires" : "Poteče",
    "No description provided" : "Anketa je brez dodatnega opisa",
    "Visible to other users" : "Vidno drugim uporabnikom",
    "Hidden to other users" : "Skrito pred drugimi uporabniki",
    "never" : "nikoli",
    "Details" : "Podrobnosti",
    "Configuration" : "Nastavitve",
    "Options" : "Možnosti",
    "Shares" : "Souporaba",
    "As an admin you may edit this poll" : "Kot skrbnik lahko urejate to anketo",
    "Description" : "Opis",
    "Poll configurations" : "Nastavitve ankete",
    "Allow admins to edit this poll" : "Dovoli skrbnikom urejanje ankete",
    "Allow \"maybe\" vote" : "Dovoli izbiro » Mogoče «",
    "Anonymous poll" : "Anonimna anketa",
    "Result display" : "Prikaži odzive",
    "Always show results" : "Vedno pokaži odzive",
    "Hide results until poll is expired" : "Skrij odzive, dokler anketa ne poteče",
    "Never show results" : "Nikoli ne pokaži odzivov",
    "Expiration date" : "Datum preteka",
    "Writing poll" : "Zapisovanje ankete",
    "Update poll" : "Posodobi anketo",
    "Title must not be empty!" : "Polje naslova ne sme biti prazno!",
    "%n successfully saved" : "Anketa %n je uspešno shranjena",
    "Add a date option" : "Dodaj možnost datuma",
    "Shift all date options" : "Premakni vse časovne možnosti za",
    "Shift" : "Premakni",
    "Available Options" : "Razpoložljive možnosti",
    "Delete option" : "Izbriši možnost",
    "Clone option" : "Kloniraj možnost",
    "Clone to option sequence" : "Kloniraj v zaporedje možnosti",
    "Step width: " : "Širina koraka:",
    "Step unit: " : "Enota koraka:",
    "Number of items to create: " : "Število ustvarjenih postavk:",
    "OK" : "V redu",
    "Week" : "Teden",
    "Minute" : "Minuta",
    "Hour" : "Ura",
    "Day" : "Dan",
    "Month" : "Mesec",
    "Year" : "Leto",
    "Click to add a date" : "Kliknite za dodajanje datuma",
    "Add a new text option" : "Dodaj novo besedilno možnost",
    "Enter option text" : "Vpis besedila možnosti",
    "Invitations" : "Povabila",
    "Invited users will get informed immediately via email!" : "Povabljeni uporabniki bodo prejeli elektronsko sporočilo takoj!",
    "Copy link to clipboard" : "Kopiraj povezavo v odložišče",
    "Remove share" : "Odstrani souporabo",
    "Public shares" : "Javne objave",
    "Public link (" : "Javna povezava (",
    "Add a public link" : "Dodaj javno povezavo",
    "Enter a name to start the search" : "Vpišite ime za začetek iskanja",
    "Link copied to clipboard" : "Povezava je kopirana v odložišče",
    "Error while copying link to clipboard" : "Prišlo je do napake med kopiranjem povezave v odložišče",
    "Unknown user" : "Nepoznan uporabnik",
    "Group" : "Skupina",
    "Public share" : "Javna objava",
    "Error while adding share" : "Napaka med omogočanjem javne objave",
    "Receive notification email on activity" : "Pošlji obvestilo o dejavnosti na anketi na elektronski naslov",
    "Your personal link to this poll: %n" : "Osebna povezava na anketo: %n",
    "Enter your name!" : "Vpišite ime!",
    "To participate, enter a username with at least 3 characters." : "Za izpolnjevanje vpišite veljavno uporabniško ime z vsaj tremi znaki.",
    "Enter your name" : "Vpišite ime",
    "Username is not valid. Please enter at least 3 characters." : "Uporabniško ime ni veljavno. Vpisati je treba vsaj tri znake.",
    "This username is not valid, i.e. because it is already in use." : "Uporabniško ime ni veljavno, ker je že v uporabi.",
    "You have an account? Log in here." : "Ali imate račun? Najprej se prijavite.",
    "Please use at least 3 characters for your username!" : "Uporabite vsaj tri znake za uporabniško ime!",
    "This username can not be chosen." : "Tega uporabniškega imena ni mogoče izbrati.",
    "Error saving username" : "Napaka shranjevanja uporabniškega imena",
    "Delete votes" : "Izbriši že izpolnjene odzive",
    "Do you want to remove {username} from poll?" : "Ali želite odstraniti uporabnika {username} z ankete?",
    "No" : "Ne",
    "Yes" : "Da",
    "The poll does not exist" : "Anketa ne obstaja.",
    "Enter a poll or start a new one." : "Vpišite naziv ankete ali pa ustvarite novo.",
    "Goto Nextcloud" : "Skočite na Nextcloud",
    "Relevant polls" : "Pomembne ankete",
    "Hidden polls" : "Skrite ankete",
    "My deleted polls" : "Izbrisane ankete",
    "Participated by me" : "Moji odzivi",
    "This are your polls (where you are the owner)." : "To so ankete, katerih lastnik ste",
    "This are all polls which are relevant or important to you, because you are a participant or the owner or you are invited to. Without expired polls." : "Izpisane so ankete, ki so vam pomembne, saj ste jih ustvarili (ali vem je dodeljeno lastništvo) oziroma ste anketo že izpolnjevali. Potečenih anket ni na  seznamu.",
    "A complete list with all public polls on this site, regardless who is the owner." : "Celoten seznam javnih anket neupoštevajoč lastništvo",
    "These are all hidden polls, to which you have access." : "Seznam skritih ankete, do katerih imate dostop.",
    "This is simply the trash bin." : "To je le smetnjak.",
    "All polls, where you placed a vote." : "Vse ankete, ki ste jih izpolnjevali.",
    "Polls which reached their expiry date." : "Ankete, ki so presegle datum veljave.",
    "All polls, where you have access to." : "Vse ankete, do katerih imate dostop",
    "Switch view" : "Preklopi pogled",
    "Toggle Sidebar" : "Preklopi bočno okno",
    "Expired" : "Poteklo",
    "Deleted" : "Izbrisano",
    "There are no vote options, add some in the options section of the right side bar." : "Ni še dodanih vprašanj. Dodati jih je mogoče iz izbire v desnem bočnem oknu.",
    "There are no vote options. Maybe the owner did not provide some until now." : "Ni še dodanih možnosti. Morda jih avtor še ni uspel dodati.",
    "Place your votes until %n" : "Anketo izpolnite do %n",
    "Public access" : "Javni dostop",
    "Hidden poll" : "Skrita anketa",
    "Error deleting poll." : "Napaka brisanja ankete",
    "Deleted poll permanently." : "Trajno izbrisana anketa.",
    "Error cloning poll." : "Napaka kloniranja ankete.",
    "Invitation mail sent to %n." : "Poslano je vabilo za %n.",
    "Error sending invitation mail to %n." : "Prišlo je do napake med pošiljanjem vabila za %n.",
    "User {userId} removed" : "Uporabnik {userId} je odstranjen",
    "Access denied" : "Dostop je zavrnjen",
    "You are not allowed to view this poll or the poll does not exist." : "Za ogled te ankete nimate ustreznih dovoljenj, ali pa anketa ne obstaja.",
    "You are not allowed to edit this poll or the poll does not exist." : "Za urejanje te ankete nimate ustreznih dovoljenj, ali pa anketa ne obstaja.",
    "You are either not allowed to delete this poll or it doesn't exist." : "Za brisanje te ankete nimate ustreznih dovoljenj, ali pa anketa ne obstaja."
},
"nplurals=4; plural=(n%100==1 ? 0 : n%100==2 ? 1 : n%100==3 || n%100==4 ? 2 : 3);");
