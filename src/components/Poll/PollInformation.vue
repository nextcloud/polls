...
import PrivatePollIcon from 'vue-material-design-icons/KeyOutline.vue'
	...
	<div class="poll-information">
		...
		<BadgeDiv v-if="pollStore.configuration.maxVotesPerUser">
			...
			{{
				n(
					'polls',
					'{usedVotes} of %n vote left.',
					'{usedVotes} of %n votes left.',
					pollStore.configuration.maxVotesPerUser,
					{ usedVotes: countUsedVotes },
				)
			}}
		</BadgeDiv>
		<BadgeDiv v-if="pollStore.configuration.maxVotesPerOption">
			...
			{{
				n(
					'polls',
					'Only %n vote per option.',
					'Only %n votes per option.',
