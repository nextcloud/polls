...
<script setup lang="ts">
	...
	const isNoAccessSet = computed(() => pollStore.configuration.access === AccessType.Private && !sharesStore.hasShares && pollStore.permissions.edit)
	...
	const subTexts = computed(() => {
		...
		if (pollStore.configuration.access === AccessType.Private) {
			subTexts.push({
				...
				text: t('polls', 'A private poll from {name}', { name: pollStore.owner.displayName }),
				class: '',
				iconComponent: null,
			})
		} else {
			subTexts.push({
				id: pollStore.configuration.access,
				text: t('polls', 'An openly accessible poll from {name}', { name: pollStore.owner.displayName }),
				class: '',
				iconComponent: null,
			})
		}
		...
		if (!pollStore.isClosed && pollStore.configuration.expire) {
			subTexts.push({
				id: 'expiring',
				text: t('polls', 'Closing {relativeExpirationTime}', { relativeExpirationTime: timeExpirationRelative.value }),
		...
		if (pollStore.isProposalExpirySet && pollStore.isProposalExpired) {
			subTexts.push({
				id: 'expired',
				text: t('polls', 'Proposal period ended {timeRelative}', { timeRelative: pollStore.proposalsExpireRelative }),
				class: 'proposal',
		...
		if (pollStore.isProposalExpirySet && !pollStore.isProposalExpired) {
			subTexts.push({
				id: 'proposal-open',
				text: t('polls', 'Proposal period ends {timeRelative}', { timeRelative: pollStore.proposalsExpireRelative }),
				class: 'proposal',
				iconComponent: ProposalsIcon,
			})
			return subTexts
	...
	const closeToClosing = computed(() => (!pollStore.isClosed && pollStore.configuration.expire && moment.unix(pollStore.configuration.expire).diff() < 86400000))
