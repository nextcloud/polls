...
<script setup lang="ts">
	...
	function calculateShiftedDate(date: Date, sequence: Sequence):Date {
		if (sequence.unit.key === DateUnitKeys.Day) {
			...
			date.setMinutes(date.getMinutes() + sequence.repetitions * sequence.stepWidth)
			...
			date.setFullYear(date.getFullYear() + sequence.repetitions * sequence.stepWidth)
		}
		return date
	}
	...
	const lastRepetitionFrom = computed(() => {
		...
		const date = calculateShiftedDate(new Date(finalFrom.value.dateTime), sequence.value)
		return date
...
<template>
	...
	<div>
		<NcCheckboxRadioSwitch v-model="useRepetition"
			type="switch">
			{{ t('polls', 'Repetitions') }}
		</NcCheckboxRadioSwitch>
	</div>
