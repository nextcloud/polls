...
<?php endif; ?>
    ...
    <div class="new_poll row">
        ...
        <div class="col-20">
			...
			<div>
            <input type="radio" class="radio" name="accessType" id="private" value="registered" <?php if(!$isUpdate || $access === 'registered') print_unescaped('checked'); ?> />
            <label for="private"><?php p($l->t('Registered users only')); ?></label>
			</div>

			<div>
            <input type="radio" class="radio" name="accessType" id="hidden" value="hidden" <?php if($isUpdate && $access === 'hidden') print_unescaped('checked'); ?> />
            <label for="hidden"><?php p($l->t('hidden')); ?></label>
			</div>

			<div>
            <input type="radio" class="radio" name="accessType" id="public" value="public" <?php if($isUpdate && $access === 'public') print_unescaped('checked'); ?> />
            <label for="public"><?php p($l->t('Public access')); ?></label>
			</div>

			<div>
            <input type="radio" class="radio" name="accessType" id="select" value="select" <?php if($isUpdate && $access === 'select') print_unescaped('checked'); ?>>
			...
			<div>
				...
				<label for="isAnonymous"><?php p($l->t('Anonymous')) ?></label>
            <div id="anonOptions" style="display:none;">
                ...
                <label for="hideNames"><?php p($l->t('Hide user names for admin')) ?></label>
            </div>
			</div>
			...
			<div class="expirationView subView">
				...
				<label for="id_expire_set"><?php p($l->t('Set expiration date')); ?></label>
        ...
        <div class="col-40">
            ...
            <input type="radio" class="radio" name="pollType" id="event" value="event" <?php if(!$isUpdate || $poll->getType() == '0') print_unescaped('checked'); ?> />
            <label for="event"><?php p($l->t('Event schedule')); ?></label>

            <!-- TODO texts to db -->
            <input type="radio" class="radio" name="pollType" id="text" value="text" <?php if($isUpdate && $poll->getType() == '1') print_unescaped('checked'); ?>>
            ...
            <div id="text-select-container" <?php if(!$isUpdate || $poll->getType() == '0') print_unescaped('style="display:none;"'); ?> >
                ...
                <div class="input-group">
                    ...
                    <div class="input-group-btn">
                        <input type="button" class="button btn" id="text-submit" value="<?php p($l->t('Add')); ?>" class="btn"/>
                    </div>
                </div>
    ...
    <div class="form-actions">
        <?php if($isUpdate): ?>
            <input type="submit" class="button btn primary" id="submit_finish_poll" value="<?php p($l->t('Update poll')); ?>" />
        <?php else: ?>
            <input type="submit" class="button btn primary" id="submit_finish_poll" value="<?php p($l->t('Create poll')); ?>" />
        <?php endif; ?>
        <a href="<?php p($urlGenerator->linkToRoute('polls.page.index')); ?>" id="submit_cancel_poll" class="button btn"><?php p($l->t('Cancel')); ?></a>
    </div>
</form>
</div>
</div>
